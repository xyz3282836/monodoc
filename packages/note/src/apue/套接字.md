...
accept 之前一样 => sockfd
不用 accept，用 recvfrom 阻塞，recvfrom 通常用于无连接的套接字
...
本质就是 bind 绑定参数结构体中的 sun_path 是一个.sock 文件路径，与网络 ipc 中绑定网络地址不同
...
在两个进程之间传送打开文件描述符的技术是非常有用的，就是使用 sendmsg 和 recvmsg 函数支持 fd 发送接受
